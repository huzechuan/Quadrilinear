...
Model Options: Trilinear Bilinear ThreeBilinear Qualinear Pentalinear
...
corpus = 'icbu-new'  # patience
word_embedding = None#'conll_german'
...
def run_command(gpu, model_path,
    ...
    model_path = os.path.join(model_path, '_'.join([str(f'L2={L2}'), emb, str(learning_rate), str(dim), str(neighbor), str(f'norm={normalize}')]))
    cmds = ''
    ...
    for lens in range(length):
        cmd = CMD_TMPLATE.format(gpu, model_path,
                                 ...
                                 hidden_size, dim,
                                 learning_rate, corpus,
                                 ranks[lens], std, use_which, L2, neighbor)
        # if lens == 0:
        #     rest = 4
        #     cmds += (cmd + '\n') * rest
        #     print(f'{cmd} * {rest}')
        ...
        print(f'{cmd} * {rounds}')
    # print(cmds)
    sh_params = f'{tag_type}_{use_which}_{corpus}_{emb}_L2={L2}_{learning_rate}_{dim}_{neighbor}_{gpu}_norm={normalize}'
