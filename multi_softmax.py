...
corpus = ['conll_03_english', 'conll_03_german', 'conll_03_dutch', 'conll_03_spanish', 'conll_03_c']  # language
word_embeddings = ['conll_english', 'conll_german', 'conll_dutch', 'conll_spanish', 'zh'] #'conll_spanish'  # language
char_embedding = False
bert_embeddings = ['bert-base-cased', 'bert-base-german-cased', 'bert-base-multilingual-cased', 'bert-base-multilingual-cased', 'bert-base-chinese']
...
def run_command(gpu, model_path,
    ...
    if percent is not 1:
        emb += str(percent)
    model_path = os.path.join(model_path, '_'.join([str(f'L2={L2}'), emb, str(learning_rate), str(gpu)]))
    cmds = ''
    ...
    sh_params = f'{tag_type}_softmax_{corpu}_{emb}_L2={L2}_{learning_rate}'
