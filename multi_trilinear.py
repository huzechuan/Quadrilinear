...
Model Options: Trilinear Bilinear ThreeBilinear Qualinear FullTrilinear ConcatScore
...
def run_command(gpu, model_path,
    ...
    if num_layers is not 1:
        emb += '_layer=2'
    model_path = os.path.join(model_path, '_'.join([emb, str(learning_rate), str(rank), str(dim), str('clone'), str(std)]))
    cmds = ''
    ...
    sh_params = f'{tag_type}_{use_which}_{corpu}_{emb}_{learning_rate}_{rank}_{gpu}'
